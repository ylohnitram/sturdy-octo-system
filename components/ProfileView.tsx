    ...
    <div className="flex flex-col h-full pb-20 pt-4 px-4 max-w-md mx-auto overflow-y-auto no-scrollbar">
        ...
        <div className="mb-8">
            <div className="flex justify-between items-center mb-3">
                ...
                <button onClick={() => fileInputRef.current?.click()} className="text-xs text-red-500 font-bold flex items-center gap-1">
                    <Plus size={14} /> Přidat fotku
                </button>
            </div>
            <div className="grid grid-cols-3 gap-2">
                <div className="aspect-square rounded-xl bg-slate-800 overflow-hidden relative group">
                    <img src="https://picsum.photos/200/300?r=1" className="w-full h-full object-cover" alt="User" />
                </div>
                <div className="aspect-square rounded-xl bg-slate-800 overflow-hidden relative group">
                    <img src="https://picsum.photos/200/300?r=2" className="w-full h-full object-cover" alt="User" />
                </div>
                <div 
                    onClick={() => unlockGallery(1)}
                    className="aspect-square rounded-xl bg-slate-900 border border-slate-800 flex flex-col items-center justify-center relative overflow-hidden cursor-pointer hover:border-red-500 transition-colors"
                >
                    {unlockedGalleries.includes(1) ? (
                         <img src="https://picsum.photos/200/300?r=private" className="w-full h-full object-cover" alt="Private" />
                    ) : (
                        <>
                            <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10"></div>
                            <Lock size={20} className="text-slate-600 mb-1" />
                            <span className="text-[10px] text-slate-500 font-bold">Soukromé</span>
                            <div className="mt-2 text-[9px] bg-yellow-500/20 text-yellow-500 px-2 py-0.5 rounded border border-yellow-500/30 font-bold">
                                20 Coins
                            </div>
                        </>
                    )}

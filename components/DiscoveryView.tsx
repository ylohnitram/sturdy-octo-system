...
import { fetchDiscoveryCandidates, updateRadarRadius, fetchUserData, sendLike } from '../services/userService';
import { supabase } from '../services/supabaseClient';

const HOTSPOTS = [
    { id: 1, name: 'Klub Nebe', distance: '0.8km', count: 42, label: 'Vysoký výskyt' },
    { id: 2, name: 'Moon Club', distance: '1.2km', count: 28, label: 'Stoupající trend' },
    { id: 3, name: 'James Dean', distance: '1.5km', count: 15, label: 'Klidnější' },
];
...
export const DiscoveryView: React.FC<DiscoveryViewProps> = ({ userStats, onConsumeAi, onConsumeCoins }) => {
  ...
  const handleSwipe = async (direction: 'left' | 'right') => {
    if (direction === 'right' && currentProfile) {
        ...
        const { data: { user } } = await supabase.auth.getUser();
        if (user) {
            await sendLike(user.id, currentProfile.id);
        }
    }

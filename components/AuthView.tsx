...
export const AuthView: React.FC<AuthViewProps> = ({ onLogin, initialView = 'signup', onBackToLanding }) => {
  ...
  const translateError = (msg: string) => {
    ...
    if (m.includes('network request failed')) return 'Chyba připojení. Zkontrolujte internet.';
          ...
          {viewState === 'signup' && (
            <div className="relative group">
              ...
              <input
                ...
                className={`w-full bg-slate-950 border rounded-xl py-3.5 pl-12 pr-10 text-white placeholder-slate-600 focus:outline-none transition-colors ${
          ...
          {viewState !== 'forgot_password' && (
            ...
            {viewState === 'signup' && (
                 <div className="relative group">
                    ...
                    className={`w-full bg-slate-950 border rounded-xl py-3.5 pl-12 pr-4 text-white placeholder-slate-600 focus:outline-none transition-colors ${
                        confirmPassword && !passwordsMatch ? 'border-red-500/50 focus:border-red-500' : 'border-slate-700 focus:border-red-500'
                    }`}
                    required
                    />
